    ...
    <div className="min-h-screen flex">
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-md animate-fade-in">
          ...
          <p className="text-on-surface-variant mb-8">Start your journey to career success.</p>

          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label htmlFor="register-name" className="block text-sm font-medium text-secondary-dark mb-1.5">Full Name</label>
              <div className="relative">
                <User size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-outline" />
                <input id="register-name" type="text" value={form.fullName} onChange={updateField('fullName')} placeholder="Your full name" className="w-full pl-10 pr-4 py-3 bg-surface border border-outline-variant/50 rounded text-sm text-on-surface placeholder:text-outline focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all" />
              </div>
            </div>

            <div>
              <label htmlFor="register-email" className="block text-sm font-medium text-secondary-dark mb-1.5">Email</label>
              <div className="relative">
                <Mail size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-outline" />
                <input id="register-email" type="email" value={form.email} onChange={updateField('email')} placeholder="Your email address" className="w-full pl-10 pr-4 py-3 bg-surface border border-outline-variant/50 rounded text-sm text-on-surface placeholder:text-outline focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all" />
              </div>
            </div>

            <div>
              <label htmlFor="register-password" className="block text-sm font-medium text-secondary-dark mb-1.5">Password</label>
              <div className="relative">
                <Lock size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-outline" />
                <input id="register-password" type={showPassword ? 'text' : 'password'} value={form.password} onChange={updateField('password')} placeholder="Create a password" className="w-full pl-10 pr-12 py-3 bg-surface border border-outline-variant/50 rounded text-sm text-on-surface placeholder:text-outline focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all" />
            ...
            <div>
              <label htmlFor="register-confirm" className="block text-sm font-medium text-secondary-dark mb-1.5">Confirm Password</label>
              <div className="relative">
                <Lock size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-outline" />
                <input id="register-confirm" type="password" value={form.confirmPassword} onChange={updateField('confirmPassword')} placeholder="Confirm your password" className="w-full pl-10 pr-4 py-3 bg-surface border border-outline-variant/50 rounded text-sm text-on-surface placeholder:text-outline focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all" />
              </div>
            </div>

            <label className="flex items-start gap-2 cursor-pointer">
              <input type="checkbox" className="w-4 h-4 mt-0.5 rounded border-outline-variant text-primary focus:ring-primary" />
              <span className="text-sm text-on-surface-variant">
                I agree to the <a href="#" className="text-primary hover:underline">Terms of Service</a> and <a href="#" className="text-primary hover:underline">Privacy Policy</a>
              </span>
            </label>

            <button type="submit" className="w-full py-3 bg-primary text-white font-semibold rounded hover:bg-primary-dark transition-all duration-200 shadow-sm hover:shadow-md">
              Create Account
            </button>
            ...
            <div className="relative flex items-center my-2">
              ...
              <span className="px-4 text-sm text-outline">Or sign up with</span>
              <div className="flex-1 border-t border-outline-variant/50"></div>
            </div>
